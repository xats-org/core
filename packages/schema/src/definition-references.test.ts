...
describe('Definition Reference Validation', () => {
  ...
  describe('XatsObject Inheritance', () => {
    it('should have XatsObject as base for all addressable objects', () => {
      ...
      objectsWithXatsBase.forEach((objName) => {
        ...
        const hasXatsObjectRef = definition.allOf.some((item: any) => item.$ref === xatsObjectRef);
  ...
  describe('StructuralContainer Inheritance', () => {
    ...
    it('should have structural containers inherit from StructuralContainer', () => {
      ...
      structuralObjects.forEach((objName) => {
        ...
        const hasStructuralRef = definition.allOf.some(
          (item: any) => item.$ref === structuralContainerRef
        );
  ...
  describe('CslDataItem External Reference', () => {
    ...
    it('should add required id field to CslDataItem', () => {
      ...
      const idRequirement = cslDataItem.allOf.find(
        (item: any) => item.required && item.required.includes('id')
      );
  ...
  describe('Content Block Type Validation', () => {
    ...
    it('should validate paragraph/heading/blockquote content structure', () => {
      ...
      expect(thenCondition.properties.content.properties.text.$ref).toBe(
        '#/definitions/SemanticText'
      );
    });
  ...
  describe('Pathway Reference Validation', () => {
    ...
    it('should validate pathway examples in trigger and pathway types', () => {
      ...
      expect(triggerExamples).toContain('https://pub.xats.org/vocabularies/triggers/onCompletion');
      expect(triggerExamples).toContain('https://pub.xats.org/vocabularies/triggers/onAssessment');

      // Check pathway type examples
      const pathwayExamples = pathway.properties.rules.items.properties.pathwayType.examples;
      expect(pathwayExamples).toContain('https://pub.xats.org/vocabularies/pathways/remedial');
      expect(pathwayExamples).toContain('https://pub.xats.org/vocabularies/pathways/standard');
      expect(pathwayExamples).toContain('https://pub.xats.org/vocabularies/pathways/enrichment');
      expect(pathwayExamples).toContain('https://pub.xats.org/vocabularies/pathways/prerequisite');
    });
  });
  ...
  describe('Matter Structure References', () => {
    ...
    it('should validate Chapter structure', () => {
      ...
      if (chapterProps.properties.learningObjectives) {
        expect(chapterProps.properties.learningObjectives.items.$ref).toBe(
          '#/definitions/LearningObjective'
        );
      }
  ...
  describe('Learning Objects References', () => {
    ...
    it('should validate LearningObjective structure', () => {
      ...
      if (objectiveProps.properties.subItems) {
        expect(objectiveProps.properties.subItems.items.$ref).toBe(
          '#/definitions/LearningObjective'
        );
      }
    });
