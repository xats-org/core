...
export class RegistryValidator {
  ...
  parseRegistryReference(reference: string): RegistryReference | null {
    ...
    // Fixed ReDoS vulnerability by limiting length and using more specific pattern
  ...
  validateRegistryReference(reference: string): { valid: boolean; errors: string[] } {
    ...
    // Validate registry name format - Fixed ReDoS by limiting length
    if (parsed.registry.length > 50 || !/^[a-z0-9][a-z0-9_-]*[a-z0-9]$/.test(parsed.registry)) {
      errors.push(
    ...
    // Validate version format (if provided) - Fixed ReDoS by limiting length and quantifiers
    if (
      parsed.version &&
  ...
  validateVersionConstraint(constraint: VersionConstraint): { valid: boolean; errors: string[] } {
    ...
    // Check for valid constraint operators and version format - Fixed ReDoS
    if (constraint.length > 100) {
      errors.push('Version constraint is too long (maximum 100 characters)');
      return { valid: false, errors };
    }
    const constraintPattern =
      /^(\^|~|>=|<=|>|<|=)?\d{1,3}\.\d{1,3}\.\d{1,3}(-[a-zA-Z0-9-]{1,20}(\.[a-zA-Z0-9-]{1,20}){0,5})?$/;
  ...
  validateMimeType(mimeType: string): { valid: boolean; errors: string[] } {
    ...
    // Fixed ReDoS by limiting MIME type length and quantifiers
    if (mimeType.length > 100) {
      errors.push('MIME type is too long (maximum 100 characters)');
      return { valid: false, errors };
    }
    const mimePattern = /^[a-z0-9][a-z0-9!#$&\-^_]{0,30}\/[a-z0-9][a-z0-9!#$&\-^_.]{0,30}$/;
