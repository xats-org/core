...
export class BibliographyProcessor {
   ...
   */
  async parseBibTeX(content: string): Promise<BibliographyEntry[]> {
   ...
   */
  renderCitation(citationRun: any, style: 'numeric' | 'author-year' | 'alpha' = 'numeric'): string {
  ...
  extractCitations(content: string): Array<{
    ...
    const citationRegex = /\\(cite[a-z]*|ref)\*?\s*(?:\[([^\]]*)\])?\s*(?:\[([^\]]*)\])?\s*\{([^}]+)\}/g;
    let match;
