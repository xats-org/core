...
class CollaborativeProjectController {
  ...
  async autoAssignRoles() {
    ...
    try {
      const response = await fetch(`${this.options.apiEndpoint}/${this.options.projectId}/auto-assign`, {
        method: 'POST',
  ...
  updateRoleAssignmentDisplay(roleId, memberIds) {
    const assignmentArea = this.projectElement.querySelector(`[data-role-id="${roleId}"] .xats-collab-assigned-members`);
    if (!assignmentArea) return;
    
    assignmentArea.innerHTML = memberIds.map(userId => {
  ...
  updateDeliverableStatusDisplay(deliverableId, status) {
    const statusArea = this.projectElement.querySelector(`[data-deliverable-id="${deliverableId}"] .xats-collab-deliverable-status`);
  ...
  async handleAssessmentSubmit(event) {
    ...
    try {
      ...
      // Submit to server
      const response = await fetch(`${this.options.apiEndpoint}/${this.options.projectId}/assessments`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          assesseeId,
  ...
  collectAssessmentData(form) {
    ...
    criterionElements.forEach(element => {
      ...
      const input = element.querySelector('input[type="range"], input[type="radio"]:checked, input[type="text"]');
