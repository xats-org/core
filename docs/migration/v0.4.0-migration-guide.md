...
@xats/renderer     - Rendering framework (HTML, Markdown, Text)
@xats/utils        - Shared utilities
@xats/examples     - Example documents
@xats/vocabularies - Vocabulary definitions (NEW)
@xats/extensions   - Extension schemas (NEW)
@xats/mcp-server   - Model Context Protocol server (coming soon)
```
...
npm install @xats/cli           # For CLI tools
npm install @xats/validator     # For validation
npm install @xats/renderer      # For rendering
npm install @xats/types         # For TypeScript types

# Or install all core packages
npm install @xats/cli @xats/validator @xats/renderer @xats/types
```
...
```typescript
import { validate } from '@xats/validator';
import { XatsDocument } from '@xats/types';
```
...
**New in v0.4.0:**
```typescript
import { HTMLRenderer, MarkdownRenderer, TextRenderer } from '@xats/renderer';
...
# from 'xats-validator' → from '@xats/validator'
# from 'xats-validator/types' → from '@xats/types'
```
...
```bash
npx @xats/cli migrate-uris ./path/to/documents
```

Or manually update URIs:
- `https://xats.org/core/blocks/*` → `https://xats.org/vocabularies/blocks/*`
- `https://xats.org/core/placeholders/*` → `https://xats.org/vocabularies/placeholders/*`
- `https://xats.org/core/hints/*` → `https://xats.org/vocabularies/hints/*`
...
// After
import type { XatsDocument, ContentBlock } from '@xats/types';
```
...
The new `@xats/renderer` package supports multiple output formats:
...
```bash
# Just validation
npm install @xats/validator @xats/types

# Just rendering
npm install @xats/renderer @xats/types

# Full suite
npm install @xats/cli
```
...
```bash
npx @xats/cli migrate-uris ./documents
```
...
# Or use npx
npx @xats/cli validate document.json
```
...
4. Check out the new examples in `@xats/examples`
