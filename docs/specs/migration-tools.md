...
function convertToCaseStudies(document) {
  const processContent = (content) => {
    return content.map(block => {
      if (block.blockType === 'https://pub.xats.org/vocabularies/blocks/paragraph' &&
...
function enhanceImageAccessibility(document) {
  const processContent = (content) => {
    return content.map(block => {
      if (block.blockType === 'https://pub.xats.org/vocabularies/blocks/figure' ||
          block.blockType === 'https://pub.xats.org/vocabularies/blocks/image') {
